...
$(document).ready(function () {
  ...
  const darkTheme = "monokai"; // or any other dark theme
  const lightTheme = "default"; // assuming 'default' is your light theme

  // Handle dark mode toggle
  $("#chk-darkmode").on("change", function () {
    const isDarkMode = storage.toggle("darkMode");
    $("body").toggleClass("dark-mode", isDarkMode);

    // Apply the theme to all CodeMirror instances
    Object.keys(editors).forEach((editorKey) => {
      const theme = isDarkMode ? darkTheme : lightTheme;
      editors[editorKey].setOption("theme", theme);
    });
  });

  // Load dark mode state on page load
  const isDarkMode = storage.get("darkMode") === "true";
  $("#chk-darkmode").prop("checked", isDarkMode);
  $("body").toggleClass("dark-mode", isDarkMode);
